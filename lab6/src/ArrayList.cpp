 ...
 */
 void ArrayList::add(string item, int position)				///position - indeks elementu po ktorym mamy wstawic
 {
	 //enlarge_x2();
//	 if(position==count_)
//		 end_=position;
 ...
 */
 void ArrayList::display(int i)				///wyswietla nazwe obiektu i numer indeksu obiektu, na ktory pokazuje jako nastepny
     {
             cout<<name_[i]<<" ";
             cout<<next_[i]<<endl;
     }
 ...
 * @return
 */
 int ArrayList::getNext(int index)
 {
	return next_[index] ;
 }
 /**
  ...
  * @return
  */
 string ArrayList::getName(int index)
 {
	 return name_[index];
 }
/**
